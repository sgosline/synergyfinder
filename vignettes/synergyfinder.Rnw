...
<<fig.show='hide'>>=
  PlotDoseResponse(dose.response.mat, save.file = TRUE, pdf.height = 6, pdf.width = 12)
@
The parameters \textit{pdf.height} and \textit{pdf.width} specify the height and width of the output pdf files. The pdf files will be saved under the current work directory with the syntax: "drug1.drug2.dose.response.blockID.pdf".
...
The PlotDoseResponse function also provides other parameters for users to adjust the output. For instance, the parameter \textit{pair.index} allows users to choose which pair of drugs to visualize and the parameters \textit{...} are further graphical parameters from the default plot function in R.
...
<<fig.show='hide'>>=
  PlotSynergy(synergy.score, type = "all", save.file = TRUE, pdf.width = 12,
              pdf.height = 10)
@
